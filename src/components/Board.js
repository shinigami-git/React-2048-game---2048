...
export default function Board(props) {
    ...
    const handleKeyDown=(event)=>{
        if (event.keyCode >= 37 && event.keyCode <= 40){
            ...
            if(event.keyCode==39){
                // console.log(gameState);
                setGameState (moveRight(gameState));
            }else if(event.keyCode==38){
                // console.log(gameState);
                setGameState (moveUp(gameState));
            }else if(event.keyCode==40){
                // console.log(gameState);
                setGameState (moveDown(gameState));
            }else{
                // console.log(gameState);
                setGameState (moveLeft(gameState));
            }
      ...
      <div className="board">
          {gameState.map((row,index)=>(
              <div className="board-row" key={index}>
                  {row.map((cell,index)=>(
                      <div className="board-box" key={index}>{cell>0 && cell}</div>
                  ))}
              </div>
          ))}
      </div>
